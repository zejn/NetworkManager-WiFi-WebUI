...
class WebUI(resource.Resource):
    ...
    def jinja2_unless_false(self, res, val):
        return res if val is not False else ''
    ...
    def handle_command(self, ev):
        ...
        if ev['q'] == 'disconnect':
            try:
                yield self.nm.wifi_disconnect(action=True)
            except self.nm.NMActionError:
                pass
            dispatch('done')
            ...
            try:
                conn_res = yield self.nm.wifi_connect(form)
    ...
    def ap_info_args(self, args):
        ...
        for k in 'connect', 'disconnect':
            ap[k] = bool(args.get(k))
        return ap
    ...
    def render_GET(self, request):
        ...
        if nm_config:
            nm_config = self.get_config_params(nm_config)
        env = dict(
    ...
    def render_POST(self, request):
        ...
        try:
            ...
            if not (ap.get('connect') or ap.get('disconnect')):
                raise KeyError(ap)
        except KeyError:
